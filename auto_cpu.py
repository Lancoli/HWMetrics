...
def start_openhardwaremonitor():
    try:
        # Assurez-vous que le chemin vers OpenHardwareMonitor.exe est correct
        openhardwaremonitor_path = "C:\\Users\\david\\Downloads\\openhardwaremonitor-v0.9.6\\OpenHardwareMonitor\\OpenHardwareMonitor.exe"
        #subprocess.Popen([openhardwaremonitor_path])
        subprocess.Popen(["run_hwm_api.bat"])
        subprocess.Popen(["runas", "/user:Administrator", "C:\\path_to_OpenHardwareMonitor\\OpenHardwareMonitor.exe"])

    except Exception as e:
        print(f"Erreur lors du lancement de OpenHardwareMonitor: {e}")
...
if __name__ == "__main__":
    start_openhardwaremonitor()  # Lancer OpenHardwareMonitor
    app = TemperatureApp()
    app.mainloop()
